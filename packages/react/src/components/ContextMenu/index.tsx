  ...
  const getMenuElement = useCallback(
    (name: string, i: number) => {
      ...
      if (name === "hide-cell") {
        ...
        // Check if any selected cell is unhide
        const hasUnhideCells = selection.row.some((r) =>
          selection.column.some((c) => {
            const cell = context.luckysheetfile[sheetIndex]?.data?.[r]?.[c];
            return cell?.hide === false || cell?.hide === undefined;
          })
        );

        // Only show lock option if there are unhide cells
        if (hasUnhideCells) {
          return (
            <Menu
              key="hide-cell"
              onClick={() => {
                setContext((draftCtx) => {
                  handleHideCell(draftCtx, refs.cellInput.current!);
                  draftCtx.contextMenu = {};
                });
              }}
            >
      ...
      if (name === "unhide-cell") {
        ...
        // Check if any selected cell is locked
        const hasHiddenCell = selection.row.some((r) =>
          selection.column.some((c) => {
            const cell = context.luckysheetfile[sheetIndex]?.data?.[r]?.[c];
        ...
        if (hasHiddenCell) {
          return (
            <Menu
              ...
              onClick={() => {
                setContext((draftCtx) => {
                  handleUnhideCell(draftCtx, refs.cellInput.current!);
                  draftCtx.contextMenu = {};
                });
              }}
            >
